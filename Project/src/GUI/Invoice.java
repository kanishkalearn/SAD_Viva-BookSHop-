...
public class Invoice extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        panel.add(jScrollPane4, new org.netbeans.lib.awtextra.AbsoluteConstraints(60, 190, 1040, 340));
    ...
    private void txt_DiscountKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_DiscountKeyReleased
        if (!txt_subTotal.getText().isEmpty()) {
            getNetTotal();
        }
        ...
        if (!qty.getText().isEmpty()) {
            ...
            for (int i = 0; i < table.getRowCount(); i++) {
                if (table.getValueAt(i, 0).equals(productCode.getText())) {
                    newQty += Double.parseDouble(table.getValueAt(i, 2).toString());
                }
            }
            if (quantity >= newQty) {
    ...
    private void qtyActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_qtyActionPerformed
        if (!qty.getText().isEmpty()) {
            if (checkAvailability.getText().equals("Available")) {
                ...
                for (int i = 0; i < table.getRowCount(); i++) {
                    if (dtm.getValueAt(i, 2).equals(productCode.getText())) {
                        sameProCode = false;
                        double addQty = Double.parseDouble(qty.getText());
                        double getQty = Double.parseDouble(dtm.getValueAt(i, 5).toString());
                        dtm.setValueAt(String.valueOf(addQty + getQty), i, 5);
                        dtm.setValueAt(decimalformat.format((addQty + getQty) * retailPrice), i, 6);
                    }
                }
    ...
    private void getSubTotal() {
        double tot = 0;
        for (int i = 0; i < table.getRowCount(); i++) {
            tot = tot + Double.parseDouble(table.getValueAt(i, 6).toString());
        }
